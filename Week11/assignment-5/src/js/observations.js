...
function titleCase(s) {
  const words = s && s.name && s.name.split(" ");
  return (
    words &&
    words
      .map((word) => {
  ...
  return observations.map(function (observation) {
    return {
      ...
      photoUrl:
        ...
        observation.taxon.default_photo.url,
...
function filterObservations(observations) {
  return observations.filter(function (observation) {
    return observation && observation.taxon && observation.taxon.default_photo;
  });
}
...
function filterOnlyNative(observations) {
  return observations.filter(function (observation) {
    return observation && observation.isNative;
  });
}
...
function filterOnlyIntroduced(observations) {
  return observations.filter(function (observation) {
    return observation && observation.isIntroduced;
  });
}

// Given an array of observations, filter out any that aren't matching the species type
// and return the filtered array.
// supported values in speciesType: all, native and introduced
function filterBySpeciesType(observations, speciesType) {
  if (speciesType === "introduced") {
    return observations.filter(function (observation) {
      return observation && observation.isIntroduced;
    });
  } else if (speciesType === "native") {
    return observations.filter(function (observation) {
      return observation && observation.isNative;
    });
  } else {
